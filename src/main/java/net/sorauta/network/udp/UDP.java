 ...
 *
 * <p>Part of the Processing Libraries project, for the Atelier Hypermedia, art school of
 * Aix-en-Provence, and for the Processing community of course. - require Java version 1.4 or later
 * - -> http://hypermedia.loeil.org/processing/ -> http://www.processing.org/
 *
 * <p>THIS LIBRARY IS RELEASED UNDER A CREATIVE COMMONS LICENSE BY. ->
...
/**
 * Create and manage unicast, broadcast or multicast socket to send and receive datagram packets
 * over the network.
 *
 * <p>The socket type is define at his initialyzation by the passed IP address. To reach a host
 * (interface) within a network, you need to specified the kind of address:
 *
 * <ul>
 *   <li>An <b>unicast address</b> refer to a unique host within a subnet.
 *   <li>A <b>broadcast address</b> allow you to call every host within a subnet.
 *   <li>A <b>multicast address</b> allows to call a specific group of hosts within the subnet. A
 *       multicast group is specified by a IP address in the range 224.0.0.0 (reserved, should not
 *       be used) to 239.255.255.255 inclusive, and by a standard UDP port number. <br>
 *       <small>notes: the complete reference of special multicast addresses should be found in the
 *       latest available version of the "Assigned Numbers RFC" </small>
 * </ul>
 *
 * A packet sent to a unicast or broadcast address is only delivered to the host identified by that
 * address. To the contrary, when packet is send to a multicast address, all interfaces identified
 * by that address receive the data .
 *
 * <p>To perform actions on receive and/or timeout events, you must implement specific method(s) in
 * your code. This method will be automatically called when the socket receive incoming data or a
 * timeout event. By default, the "receive handler" is typically <code>receive(byte[] data)</code>
 * but you can retrieve more informations about the datagram packet, see {@link
 * UDP#setReceiveHandler(String name)} for more informations. In the same logic, the default
 * "timeout handler" is explicitly <code>timeout()</code>.
 *
 * <p><small> note: currently applets are not allowed to use multicast sockets </small>
 *
 * @version 0.1
...
public class UDP implements Runnable {
  ...
  /**
   * Create a new datagram socket and binds it to an available port and every address on the local
   * host machine.
   *
   ...
   *
   * <p>Pass <code>zero</code> as port number, will let the system choose an available port.
   *
   * @param owner the target object to be call by the receive handler
  ...
  /**
   * Create a new datagram socket and binds it to the specified port on the specified local address
   * or multicast group address.
   *
   * <p>Pass <code>zero</code> as port number, will let the system choose an available port. The
   * absence of an address, explicitly <code>null</code> as IP address will assign the socket to the
   * Unspecified Address (Also called anylocal or wildcard address). To set up the socket as
   * multicast socket, pass the group address to be joined. If this address is not a valid multicast
   * address, a broadcast socket will be created by default.
   *
   * @param owner the target object to be call by the receive handler
   * @param port local port to bind
  ...
  /**
   * Close the socket. This method is automatically called by Processing when the PApplet shuts
   * down.
   *
   * @see UDP#close()
   */
  ...
  /**
   * Return the actual socket's local address, or <code>null</code> if the address correspond to any
   * local address.
   *
   * @return String
   */
  public String address() {
    ...
    InetAddress laddr = isMulticast() ? mcSocket.getLocalAddress() : ucSocket.getLocalAddress();
    return laddr.isAnyLocalAddress() ? null : laddr.getHostAddress();
  }

  /**
   * Send message to the current socket. Explicitly, send message to the multicast group/port or to
   * itself.
   *
  ...
  /**
   * Send data to the current socket. Explicitly, send data to the multicast group/port or to
   * itself.
   *
   ...
   *
   * <p>A <code>null</code> IP address will assign the packet address to the local host. Use this
   * method to send message to another application by passing <code>null</code> as the destination
   * address.
   *
   * @param message the message to be send
   * @param ip the destination host's IP address
   ...
   *
   * <p>A <code>null</code> IP address will assign the packet address to the local host. Use this
   * method to send data to another application by passing <code>null</code> as the destination
   * address.
   *
   * @param buffer data to be send
   * @param ip the destination host's IP address
  ...
  /**
   * Set the maximum size of the packet that can be sent or receive on the current socket. This
   * value must be greater than 0, otherwise the buffer size is set to the his default value.
   *
   * <p>return <code>true</code> if the new buffer value have been correctly set, <code>false</code>
   * otherwise.
   *
   * <p><i>note : this method has no effect if the socket is listening. To define a new buffer size,
   * call this method before implementing a new buffer in memory. Explicitly before calling a
   * receive methods.</i>
   *
  ...
  /**
   * Set the socket reception timeout and wait one time for incoming data. If the timeout period
   * occured, the owner timeout() method is automatically called.
   *
  ...
  /**
   * Wait for incoming data, and call the appropriate handlers each time a message is received. If
   * the owner's class own the appropriate target handler, this method send it the receive message
   * as byte[], the sender IP address and port.
   *
   * <p>This method force the current <code>Thread</code> to be ceased for a temporary period. If
   * you prefer listening without blocking the current thread, use the {@link UDP#listen(int
   * millis)} or {@link UDP#listen(boolean on)} method instead.
   *
   * @see UDP#listen()
   * @see UDP#listen(boolean on)
   * @see UDP#setReceiveHandler(String name)
  ...
  /**
   * Wait for incoming datagram packets. This method is called automaticlly, you do not need to call
   * it.
   */
  public void run() {
    while (listen) listen();
   ...
   *
   * <p>By default, this method name is "receive" with one argument representing the received data
   * as <code>byte[]</code>. For more flexibility, you can change this method with another useful
   * method by passing his name. You could get more informations by implementing two additional
   * arguments, a <code>String</code> representing the sender IP address and a <code>int</code>
   ...
   */
  private void callReceiveHandler(byte[] data) throws NoSuchMethodException {
  ...
  /**
   * Register the target's timeout handler. By default, this method name is "timeout" with no
   * argument.
   *
  ...
  /**
   * Call the timeout target method when the socket received a timeout event. The method name to be
   * implemented in your code is <code>timeout()</code>.
   */
  ...
  /**
   * Enable or disable the multicast socket loopback mode. By default loopback is enable. <br>
   * Setting loopback to false means this multicast socket does not want to receive the data that it
   * sends to the multicast group.
   *
  ...
  /**
   * Control the life-time of a datagram in the network for multicast packets in order to indicates
   * the scope of the multicasts (ie how far the packet will travel).
   *
   * <p>The TTL value must be in range of 0 to 255 inclusive. The larger the number, the farther the
   * multicast packets will travel (by convention):
   *
   * <blockquote>
   *
   * <pre>
   ...
   *
   * <p>return <code>true</code> if no error occured.
   *
  ...
  /**
   * Return the "Time to Live" value or -1 if an error occurred (or if the current socket is not a
   * multicast socket).
   *
   * @return int
